...
with st.sidebar:
    ...
    if st.session_state.model_artifacts:
        ...
        _ver = st.session_state.model_artifacts.get("model_version", "?")
        st.caption(f"Model version: **{_ver}**")
        st.caption("Stack: XGBoost · LightGBM · Random Forest · Neural Net · Isolation Forest")
    else:
...
with tab_upload:
    ...
    uploaded_files = st.file_uploader(
        ...
        help="Select several CSVs at once (Ctrl/Shift-click or drag multiple). "
             "They are validated, combined and scored as a single batch. "
             "Each file keeps its own audit-trail source."
    )

    # Validate and combine every selected file. Per-file status is surfaced so
    # a bad file never silently blocks the good ones.
    df_upload = None
    file_status = []
    ...
    if uploaded_files:
        ...
        for uf in uploaded_files:
            try:
                ...
                file_status.append(f"⚠️ **{uf.name}**: could not be read ({e}) — skipped")
                continue
            missing = validate_csv(part)
            if missing:
                file_status.append(
                    f"⚠️ **{uf.name}**: missing columns "
                    f"{', '.join(missing)} — skipped"
                )
                continue
            part = part.copy()
            part["_source_file"] = uf.name
            frames.append((uf.name, part))

        for name, part in frames:
            file_status.append(f"✅ **{name}**: {len(part):,} transactions")

        if frames:
            df_upload = pd.concat([part for _, part in frames], ignore_index=True)

    col_a, col_b = st.columns(2)
    with col_a:
        if df_upload is not None:
            st.success(f"Loaded {len(df_upload):,} transactions across {len(frames)} file(s)")
            st.dataframe(df_upload.head(10), width="stretch")
            for msg in file_status:
                st.markdown(msg)
        elif uploaded_files:
            st.warning("None of the selected files could be used. See missing-column errors above.")
            for msg in file_status:
                st.markdown(msg)
    ...
    if df_upload is not None and st.button("🔍 Run Fraud Detection", type="primary", width="stretch"):
        with st.spinner(f"Scoring {len(df_upload):,} transactions in {currency}..."):
        ...
        # Persist every decision to the automated audit trail (USD base). Each
        # row is attributed to its originating file via the _source_file column.
        n_logged = record_batch(results, data_source="CSV upload",
                                currency=currency, rate=rate)
        # Convert results back to the selected currency for display
        results = df_to_local(results, rate)
        results = results.drop(columns=["_source_file"])
    ...
    if st.session_state.results_df is not None:
        ...
        explain(
            ...
            - **score_xgb, score_lgbm, score_rf, score_nn, score_iso** — how each of the
              five ensemble members voted *before* weighing. A big gap between members
              (e.g. XGBoost 0.9 but Random Forest 0.2) means the models disagree — worth
              a closer look, since consensus is more trustworthy than disagreement.
...
with tab_analytics:
    ...
    st.markdown("### Model Comparison (Per-Member ROC-AUC)")
    model_aucs = metrics.get("model_aucs", {})
    if model_aucs:
        auc_df = pd.DataFrame({
            "Model": list(model_aucs.keys()),
            "ROC-AUC": list(model_aucs.values()),
        })
        fig_auc = px.bar(
            auc_df, x="Model", y="ROC-AUC",
            color="ROC-AUC", color_continuous_scale="Blues",
            text_auto=".3f", range_y=[0.5, 1.0],
            title="Which model contributes the most?"
        )
        ...
        explain(
            ...
            """
            This chart shows each ensemble member's individual ROC-AUC on the held-out test
            set (Ensemble = their weighted combination). Use it to judge *contribution*:

            - A member near the Ensemble's AUC is carrying its weight well.
            - A member clearly below the others still adds value if it oners something the
              others miss — Isolation Forest is run **unsupervised**, so it can catch novel
              patterns nobody labelled before.
            - If you ever retrain, this is your health check: any member crashing in AUC
              should be re-tuned or replaced.
            """
        )
    opt = metrics.get("optimal_threshold")
    if opt is not None:
        if threshold > opt:
            advice = "Consider lowering the sidebar threshold toward the optimum to catch more fraud."
        else:
            advice = "Your threshold is at/below the optimum — you favour catching fraud over precision, which is safe for screening."
        st.info(
            f"The statistically optimal flagging threshold (Youden's J — maximises "
            f"sensitivity + specificity) is **{opt:.3f}**. You are currently using "
            f"**{threshold:.2f}**. {advice}"
        )
...
with tab_about:
    ...
    ### How It Works
    1. **Data Input**: Upload one or more CSVs or enter a transaction manually
    ...
    3. **Feature Engineering**: 12+ derived features from raw transaction attributes
       (ratios, cyclical time encoding, overdraft flags)
    4. **Modern 5-Model Ensemble**: XGBoost (30%) + LightGBM (25%)
       + Random Forest (15%) + Neural Net (10%) + Isolation Forest (20%) vote together
    ...
    |-----------|---------|--------|
    | XGBoost | Gradient-boosted trees | 30% |
    | LightGBM | Fast gradient boosting (MS LightGBM) | 25% |
    | Random Forest | Ensemble bagging for robust predictions | 15% |
    | Neural Network (MLP) | Learns non-linear interactions between features | 10% |
    ...
    **The 4-step workflow**
    ...
    2. **Model scores** — a modern ensemble of five models votes (XGBoost 30%, LightGBM
       25%, Random Forest 15%, Neural Net 10%, Isolation Forest 20%); the result is one
       score per transaction. You can see each member's individual vote in the `score_*`
       columns and compare their performance in the Analytics tab.
