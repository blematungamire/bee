...
class TestFeatureEngineering:
    ...
    def test_cyclical_hour_encoding_valid(self):
        df = pd.DataFrame({"hour_of_day": [0, 12, 23], "amount": [10]*3,
                           "balance_before": [1000]*3, "balance_after": [990]*3,
                           "day_of_week": ["Mon"]*3})
        result = engineer_features(df)
        # Hours 0 and 23 are adjacent on the 24h circle (sep = 2pi/24),
        # while 0 and 12 are opposite (sep = pi).
        def circ_dist(a, b):
            return np.sqrt((a[0] - b[0]) ** 2 + (a[1] - b[1]) ** 2)
        p0 = (result["hour_sin"].iloc[0], result["hour_cos"].iloc[0])
        p12 = (result["hour_sin"].iloc[1], result["hour_cos"].iloc[1])
        p23 = (result["hour_sin"].iloc[2], result["hour_cos"].iloc[2])
        assert circ_dist(p0, p23) < circ_dist(p0, p12)
        assert result["hour_sin"].between(-1, 1).all()
        assert result["hour_cos"].between(-1, 1).all()

    def test_negative_balance_flag(self):
        df = pd.DataFrame({"hour_of_day": [12], "amount": [10],
                           "balance_before": [100], "balance_after": [-50],
                           "day_of_week": ["Mon"]})
        result = engineer_features(df)
        assert result["is_negative_balance"].iloc[0] == 1
...
class TestModelTraining:
    def test_returns_expected_keys(self, trained_artifacts):
        required_keys = {"xgb_model", "lgbm_model", "rf_model", "nn_model",
                         "iso_model", "scaler", "encoders", "metrics",
                         "model_version", "ensemble_weights"}
        assert required_keys.issubset(set(trained_artifacts.keys()))
    ...
    def test_per_model_aucs_present(self, trained_artifacts):
        aucs = trained_artifacts["metrics"]["model_aucs"]
        for name in ["XGBoost", "LightGBM", "Random Forest", "Neural Net",
                     "Isolation Forest", "Ensemble"]:
            assert name in aucs, f"missing AUC for {name}"
            assert 0 <= aucs[name] <= 1

    def test_optimal_threshold_valid(self, trained_artifacts):
        t = trained_artifacts["metrics"]["optimal_threshold"]
        assert 0 < t < 1

    def test_ensemble_weights_sum_to_one(self, trained_artifacts):
        w = trained_artifacts["ensemble_weights"]
...
class TestPredictionNormal:
    ...
    def test_returns_member_scores(self, trained_artifacts, sample_batch):
        """predict() should expose each ensemble member's vote."""
        result = predict(sample_batch, trained_artifacts)
        for col in ["score_xgb", "score_lgbm", "score_rf", "score_nn", "score_iso"]:
